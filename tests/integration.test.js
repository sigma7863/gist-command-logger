...
import { cmdInit, cmdRecordFinish, runUpload } from "../src/commands.js";
import { loadConfig, writeJsonFile, loadState } from "../src/fs.js";
...
test("init writes zsh block and run-upload uploads filtered records", () => {
  ...
  assert.match(zshrc, /gist-command-logger/);
  ...
  assert.equal(Object.keys(db.gists).length, 1);

  const state = loadState(paths.statePath);
  const logLines = fs.readFileSync(paths.logPath, "utf8").trim().split(/\r?\n/);
  assert.equal(state.lastUploadedLine, logLines.length);
});
