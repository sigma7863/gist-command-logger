...
import { CONFIG_MARKERS } from "./constants";

function getHomeDir(): string {
  return process.env.GCL_HOME || os.homedir();
}

function upsertBlock(filePath: string, startMarker: string, endMarker: string, blockContent: string): void {
...
function zshBlock(binName: string): string {
...
function bashBlock(binName: string): string {
...
export function installHooks(shell: "zsh" | "bash", binName = "gist-command-logger"): string {
...
export function detectShell(inputShell = process.env.SHELL || ""): "zsh" | "bash" {
